...
EC$(function ($) {
  ...
  function sale_percent() {
    ...
    if (mainEl.length > 0) {
      var productListEl = $(
        '.xans-product-listmain, .xans-product-listrecommend, .xans-product-listnew, .xans-product-listnormal, .xans-search-result'
      );
  ...
  function percentageCul(target, salePriceEl) {
    ...
    if (iCustomPrice > 0 && iPrice > 0 && iPrice != iCustomPrice) {
      ...
      salePriceEl.prepend("<div class='sale_box'>" + sSaleText + '</div>');
    }
  }
